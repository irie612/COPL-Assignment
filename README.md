...
The expr_p() function does this whole process again, in the instance that there are more correct expressions. However, the expr_p() function does not necessarily need to add anything, and can essentially be "empty". If called, the program would return to the previous operation in the stack.

After every token has been analyzed, and the line has been parsed, the parse() function outputs the string. If after every correct expression has been analyzed and parsed, the token does not equal to either EOL or EOF, then the parse() function will exit with status 1 (string not fully parsed). 
